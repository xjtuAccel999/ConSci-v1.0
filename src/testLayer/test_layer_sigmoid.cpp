...
void test_layer_sigmoid() {
    ...
    ncnn::Option opt;
    ncnn::Mat data_in = RandomMat(10, 150, 100);
    ...
    #ifdef ALU_TIME
		u64 tEnd_sigmoid_sw, tCur_sigmoid_sw;
		u32 tUsed_sigmoid_sw;
        XTime_GetTime(&tCur_sigmoid_sw);
    #endif
    forward_sim_sigmoid(data_in, c_sim);
    #ifdef ALU_TIME
		XTime_GetTime(&tEnd_sigmoid_sw);
		tUsed_sigmoid_sw = ((tEnd_sigmoid_sw-tCur_sigmoid_sw)*1000000)/(COUNTS_PER_SECOND);
		printf("time_sigmoid_sw elapsed is %d us\n",tUsed_sigmoid_sw);
    #endif
