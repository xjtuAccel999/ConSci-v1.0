...
static int detect_yolov3(const cv::Mat& bgr, std::vector<Object>& objects)
{
    ...
    if (yolov3.load_param_mem(load_file_align("yolov3_int8.param")))
        exit(-1);
    if (yolov3.load_model_mem(load_file_align("yolov3_int8.bin")))
        exit(-1);
...
//    const int target_size = 256;
    ...
    ex.extract("output", out);
		...
		printf("time_yolov3 elapsed is %d us, fps = %.3f\r\n",tUsed,fps);
    #endif
    objects.clear();
...
int yolov3_inference()
{
    ...
    // printf("[ log ]: gemm_c_all = %d, gemm_c_all_align32 = %ld, utilization rate = %.3f\n",gemm_c_all,gemm_c_all_align32,((double)(gemm_c_all/gemm_c_all_align32))*100.f);
