...
static int detect_yolov6n(const cv::Mat& bgr, std::vector<Object>& objects, const std::vector<int>& strides)
{
    ...
    if (yolov6.load_param_mem(load_file_align("yolov6n_int8.param")))
        exit(-1);
    if (yolov6.load_model_mem(load_file_align("yolov6n_int8.bin")))
        exit(-1);
    ...
    ex.extract("outputs", out);
		...
		printf("time_yolov6n elapsed is %d us, fps = %.3f\r\n",tUsed,fps);
    #endif
