...
static int detect_resnet101(const cv::Mat& bgr, std::vector<float>& cls_scores)
{
    ...
    if (resnet101.load_param_mem(load_file_align("resnet101_int8.param")))
        exit(-1);
    if (resnet101.load_model_mem(load_file_align("resnet101_int8.bin")))
        exit(-1);
    ...
    ex.extract("out0", out);
		...
		printf("time_resnet101 elapsed is %d us, fps = %.3f\r\n",tUsed,fps);
    #endif
