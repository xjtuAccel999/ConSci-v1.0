...
static int detect_mobilenetv2(const cv::Mat& bgr, std::vector<Object>& objects)
{
    ...
    if (mobilenetv2.load_param_mem(load_file_align("mobilenetv2_yolo_int8.param")))
        exit(-1);
    if (mobilenetv2.load_model_mem(load_file_align("mobilenetv2_yolo_int8.bin")))
        exit(-1);
    ...
    ex.extract("detection_out", out);
		...
		printf("time_mobilenetv2_yolo elapsed is %d us, fps = %.3f\r\n",tUsed,fps);
    #endif
    //     printf("%d %d %d\n", out.w, out.h, out.c);
    objects.clear();
