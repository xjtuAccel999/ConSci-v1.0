...
static int detect_retinaface(const cv::Mat& bgr, std::vector<FaceObject>& faceobjects)
{
    ...
    if (retinaface.load_param_mem(load_file_align("retinaface_int8.param")))
        exit(-1);
    if (retinaface.load_model_mem(load_file_align("retinaface_int8.bin")))
        exit(-1);
