...
static int detect_resnet18(const cv::Mat& bgr, std::vector<float>& cls_scores)
{
    ...
    if (resnet18.load_param_mem(load_file_align("resnet18_int8.param")))
        exit(-1);
    if (resnet18.load_model_mem(load_file_align("resnet18_int8.bin")))
        exit(-1);
    ...
    ex.extract("out0", out);
		...
		printf("time_resnet18 elapsed is %d us, fps = %.3f\r\n",tUsed,fps);
    #endif
