...
void hw_gemm::gemm_forward_block(float* ifm_baseaddr_t, unsigned char* wgt_baseaddr_t, float* ofm_baseaddr_t) {
    ...
    while (!is_finish) {
        ...
        //     // printf("time_conv elapsed is %d us\n",tUsed);
        //     conv_time_gemm_forward_block = tUsed_conv_hw + conv_time_gemm_forward_block;
        // #endif
...
uint64_t gemm_c_all = 0;
uint64_t gemm_c_all_align32 = 0;
float util = 0.f;
int cnt = 0;
void hw_gemm::gemm_forward(ncnn::Mat &ifm, ncnn::Mat &wgt, ncnn::Mat &ofm){
    // #ifdef CONV_TIME
	// 	u64 tEnd_gemm_forward, tCur_gemm_forward;
	// 	u32 tUsed_gemm_forward;
    //     XTime_GetTime(&tCur_gemm_forward);
    // #endif
//#ifdef PRINT_CONV_TIME
//    time_dma_ch0_r = 0;
//    time_dma_ch1_r = 0;
...
//    start_time = main_time;
    ...
    printf("\n");
#endif
    // gemm_c_all = ifm_c + gemm_c_all;
    // gemm_c_all_align32 = gemm_c_all_align32 + u_align(ifm_c,32);
    // util = util + (float)ifm_c/u_align(ifm_c,32);
    float ofm_util ;
    float ifm_util ;
    if(ofm_c%32 == 0){
    	ofm_util = 1;
    }else{
    	ofm_util = ((float)(ofm_c%32)/32);
    }
    if(ifm_c%32 == 0){
    	ifm_util = 1;
        }else{
        ifm_util = ((float)(ifm_c%32)/32) ;
        }
    if((ofm_w*ofm_h)%64 >32){
    	util = util + ofm_util * ifm_util  ;
    }else {
    	util = util + ofm_util * ifm_util * 0.5f  ;
    }
    ...
    printf("[ log ]: cnt = %d, ifm_utilization rate = %.5f,ofm_utilization rate = %.5f,utilization rate = %.5f\n", cnt, ifm_util, ofm_util, util);
//    #ifdef PRINT_CONV_TIME
...
//        printf("[ log ]: time_dmasendbuf_congest = %ld, => %.3f%%\n", time_dmasendbuf_congest, (double)time_dmasendbuf_congest/(double)conv_time*100.f);
//        printf("\n\n");
//        conv_layer_count ++;
//    #endif
//    time_gemm += main_time - start_time;
    // #ifdef CONV_TIME
	// 	XTime_GetTime(&tEnd_gemm_forward);
	// 	tUsed_gemm_forward = ((tEnd_gemm_forward-tCur_gemm_forward)*1000000)/(COUNTS_PER_SECOND);
	// 	// printf("time_conv_gemm_forward elapsed is %d us , time_conv_hw elapsed is %d\n",tUsed_gemm_forward,conv_time_gemm_forward_block);
    //     conv_time_gemm_forward_block = 0;
    // #endif
}
