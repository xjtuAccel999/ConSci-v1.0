...
public:
    ...
    int fwrite_weight_tag_data_conv(const ncnn::Mat& data, FILE* bp, int ic, int oc, int kernel, int conv_type);
...
int ModelWriter::fwrite_weight_tag_data(const ncnn::Mat& data, FILE* bp, float a, float b)
{
    ...
    unsigned char padding[NCNN_MAT_ALIGN_BYTES];
    memset(padding, 0x00, NCNN_MAT_ALIGN_BYTES);
    ...
    if (data_flattened.elemsize == 4)
    {
        if (storage_type == 1)
        {
            ...
            fwrite(padding, sizeof(unsigned char), NCNN_MAT_ALIGN_BYTES-4, bp);
    ...
    // padding to 256bit align
    int nwrite = ftell(bp) - p0;
    size_t nalign = alignSize(nwrite, NCNN_MAT_ALIGN_BYTES);
...
int ModelWriter::fwrite_weight_tag_data_conv(const ncnn::Mat& data, FILE* bp, int ic, int oc, int kernel, int conv_type)
{

    //conv_type == 0  +> common conv
    //conv_type == 1  +> depth  conv
    int p0 = ftell(bp);
    unsigned char padding[NCNN_MAT_ALIGN_BYTES];
    memset(padding, 0x00, NCNN_MAT_ALIGN_BYTES);
    const int tag = 0x000D4B38; // int8 magic
    ...
    ncnn::Mat data_flattened = data.reshape(data.w * data.h * data.d * data.c);
    assert(gen_random_weight == 0);
    assert(data_flattened.elemsize == 1);

    ncnn::Mat wgt_buffer_res;
    ncnn::Mat wgt_buffer;

    if(conv_type == 0){
        wgt_buffer_res.create(u_align(oc, 32), 1, u_align(ic, 32) * kernel * kernel, 1u, opt.blob_allocator);
        wgt_buffer.create(32, u_align(ic, 32) * kernel * kernel * u_align(oc, 32) / 32, 1u, opt.blob_allocator);
        accel::weightorder wgtorder(ic, oc, kernel);
        wgtorder.wgt_reorder(data_flattened,wgt_buffer_res, ic, oc, kernel,1, 0);
        wgtorder.wgt_gen(wgt_buffer_res,wgt_buffer,ic, oc, kernel ,0,1);
    }
    else if(conv_type == 1){
        wgt_buffer_res.create(1, 1, u_align(ic, 32) * kernel * kernel, 1u, opt.blob_allocator);
        wgt_buffer.create(32, u_align(ic, 32) * kernel * kernel / 32, 1u, opt.blob_allocator);
        accel::weightorder wgtorder(ic, oc, kernel);
        wgtorder.wgt_reorder(data_flattened, wgt_buffer_res, ic, oc, kernel, 0, 1);
        wgtorder.wgt_gen(wgt_buffer_res, wgt_buffer, ic,oc,kernel, 1, 1);
    }

    ncnn::Mat wgt_order = wgt_buffer.reshape(wgt_buffer.w * wgt_buffer.h * wgt_buffer.d * wgt_buffer.c);
    fwrite(wgt_order.data, wgt_order.elemsize, wgt_order.w, bp);
...
#define fprintf_param_value(format, phase)                                  \
    {                                                                       \
        ...
        else if (layer->type == "Convolution")
        {
            ...
            fprintf_param_value(" 20=%d", layout_en)

            op->ifm_w = layer->bottom_shapes[0].w;
            op->ifm_h = layer->bottom_shapes[0].h;
            op->ifm_c = layer->bottom_shapes[0].c;

            fprintf_param_value(" 21=%d", ifm_w)
            fprintf_param_value(" 22=%d", ifm_h)
            fprintf_param_value(" 23=%d", ifm_c)
            int inc = op->weight_data_size / (op->kernel_h * op->kernel_w * op->num_output);
            if(op->weight_data.elemsize == 1){
                assert(op->kernel_h == op->kernel_w);
                fwrite_weight_tag_data_conv(op->weight_data, bp, inc, op->num_output, op->kernel_w, 0);
            }
            else{
            ...
            if (op->int8_scale_term)
            {
                ...
                ncnn::Option opt;
                ncnn::Mat dequant_scales;
                dequant_scales.create_like(op->weight_data_int8_scales, opt.blob_allocator);
                assert(dequant_scales.dims == 1);
                for (int dequant_index = 0; dequant_index < dequant_scales.w; dequant_index++)
                    dequant_scales[dequant_index] = 1.f / ((op->bottom_blob_int8_scales)[0] * (op->weight_data_int8_scales)[dequant_index]);
                fwrite_weight_data(dequant_scales, bp, 0.01, 12);
        ...
        else if (layer->type == "ConvolutionDepthWise")
        {
            ...
            fprintf_param_value(" 22=%d", ifm_h)
            fprintf_param_value(" 23=%d", ifm_c)
            int inc = op->num_output;
            // int inc = op->weight_data_size / (op->kernel_h * op->kernel_w * op->num_output);
            if(op->weight_data.elemsize == 1){
                assert(op->kernel_h == op->kernel_w);
                fwrite_weight_tag_data_conv(op->weight_data, bp, inc, op->num_output, op->kernel_w, 1);
            }
            else{
                fwrite_weight_tag_data(op->weight_data, bp);
            }
        ...
        else if (layer->type == "InnerProduct")
        {
            ...
            op->weight_data_size_align = alignSize(op->weight_data_size, NCNN_MAT_ALIGN_BYTES);
            fprintf_param_value(" 11=%d", weight_data_size_align)
