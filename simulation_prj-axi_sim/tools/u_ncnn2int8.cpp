...
int NetQuantize::fuse_requantize()
{
    ...
    for (size_t i = 0; i < layer_count; i++)
    {
        ...
        if(layers[i]->type == "Convolution"){
            ...
            if(op->num_output > 1024 || op->kernel_w*op->kernel_h*alignSize(op->ifm_c,32) > WGT_BUFFER_DEPTH)
                continue;
        }
        else if(layers[i]->type == "ConvolutionDepthWise"){
            ncnn::ConvolutionDepthWise* op = (ncnn::ConvolutionDepthWise*)layers[i];
            if(op->num_output > 1024)
                continue;
        }
...
int main(int argc, char** argv)
{
    ...
    quantizer.load_param_mem(quantizer.load_file2mem(inparam));
    ...
    if (strcmp(inbin, "null") == 0)
    {
        ...
        quantizer.load_model_mem(quantizer.load_file2mem(inbin));
