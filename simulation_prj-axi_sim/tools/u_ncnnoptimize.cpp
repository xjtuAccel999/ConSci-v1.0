...
int NetOptimize::fuse_convolution_activation()
{
    ...
    for (size_t i = 0; i < layer_count; i++)
    {
        ...
        ncnn::Convolution* op = (ncnn::Convolution*)layers[i];
        
        if(op->activation_type == 1 || op->activation_type == 2 || op->activation_type == 4 || op->activation_type == 7 || op->activation_type == 8 || op->activation_type == 9){
            if((op->activation_params).empty()){
                op->activation_params = ncnn::Mat(2);
                op->activation_params[0] = 0.f;
                op->activation_params[1] = 0.f;
            }
            else if((op->activation_params).total()==1){
                float param_temp = op->activation_params[0];
                op->activation_params = ncnn::Mat(2);
                op->activation_params[0] = param_temp;
                op->activation_params[1] = 0.f;
            }
        }
        ...
        for (; j < layer_count; j++)
        {
            if (layers[j]->type != "ReLU" && layers[j]->type != "Clip" && layers[j]->type != "Sigmoid" && layers[j]->type != "HardSwish" && layers[j]->type != "TanH" && layers[j]->type != "Swish" && layers[j]->type != "ELU" && layers[j]->type != "SELU")
                continue;
        ...
        if (activation->type == "ReLU")
        {
            ...
            if (relu->slope == 0.f)
            {
                ...
                convolution->activation_params[0] = 0.f;
                convolution->activation_params[1] = 0.f;
            }
            else
            {
        ...
        {
            convolution->activation_type = 8;
            convolution->activation_params = ncnn::Mat(2);
            convolution->activation_params[0] = 0.f;
            convolution->activation_params[1] = 0.f;
        }
        else if (activation->type == "ELU")
        {
            ncnn::ELU* elu = (ncnn::ELU*)activation;
            convolution->activation_type = 9;
            convolution->activation_params = ncnn::Mat(2);
            convolution->activation_params[0] = elu->alpha;
            convolution->activation_params[1] = 0.f;
        }
        else if (activation->type == "SELU")
        {
            ncnn::SELU* selu = (ncnn::SELU*)activation;
            convolution->activation_type = 10;
            convolution->activation_params = ncnn::Mat(2);
            convolution->activation_params[0] = selu->alpha;
            convolution->activation_params[1] = selu->lambda;
        }
...
int NetOptimize::fuse_convolutiondepthwise_activation()
{
    ...
    for (size_t i = 0; i < layer_count; i++)
    {
        ...
        ncnn::ConvolutionDepthWise* op = (ncnn::ConvolutionDepthWise*)layers[i];
        ...
        if (activation->type == "ReLU")
        {
            ...
            else
            {
                ...
                convolutiondepthwise->activation_params[1] = 0.f;
            }
        }
        else if (activation->type == "Clip")
        {
        ...
        else if (activation->type == "Sigmoid")
        {
            ...
            convolutiondepthwise->activation_params = ncnn::Mat(2);
            convolutiondepthwise->activation_params[0] = 0.f;
            convolutiondepthwise->activation_params[1] = 0.f;
        }
        // else if (activation->type == "Mish")
        // {
        ...
        {
            convolutiondepthwise->activation_type = 8;
            convolutiondepthwise->activation_params = ncnn::Mat(2);
            convolutiondepthwise->activation_params[0] = 0.f;
            convolutiondepthwise->activation_params[1] = 0.f;
        }
        ...
        {
            ncnn::ELU* elu = (ncnn::ELU*)activation;
            convolutiondepthwise->activation_type = 9;
            convolutiondepthwise->activation_params = ncnn::Mat(2);
            convolutiondepthwise->activation_params[0] = elu->alpha;
            convolutiondepthwise->activation_params[1] = 0.f; 
        }
        else if (activation->type == "SELU")
        {
            ncnn::SELU* selu = (ncnn::SELU*)activation;
            convolutiondepthwise->activation_type = 10;
            convolutiondepthwise->activation_params = ncnn::Mat(2);
            convolutiondepthwise->activation_params[0] = selu->alpha;
            convolutiondepthwise->activation_params[0] = selu->lambda;
        }
...
int main(int argc, char** argv)
{
    ...
    {
        printf("[error]: input *.bin file is empty\n");
        assert(0);
        ...
        // optimizer.load_model_mem(optimizer.load_file2mem(inbin));
    }
