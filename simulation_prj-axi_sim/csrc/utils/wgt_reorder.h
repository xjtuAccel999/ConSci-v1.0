...
// wgt_reorder flip_en = 1, op = 0
// static void wgt_reorder_conv(ncnn::Mat &i_data, ncnn::Mat &o_data, accel::hw_gemm &inst, int flip_en = 0) {
// #ifdef MAT_LOG
//     log_origin_wgt(i_data, inst);
// #endif
//     int ifm_c_align32 = ncnn::alignSize(inst.ifm_c, 32);
//     int maxk = inst.kernel * inst.kernel;
//     int oc_step = maxk * inst.ifm_c;
//     ncnn::Option opt;
//     // first step: maxk*ic*oc -> (oc,1,maxk*ic)
//     ncnn::Mat wgt_buffer(ncnn::alignSize(inst.ofm_c, 32), 1, ifm_c_align32 * maxk, 1u, opt.blob_allocator);
//     char *wgt_i_ptr = i_data;
//     char *wgt_o_ptr = wgt_buffer;
//     memset(wgt_o_ptr, 0, sizeof(char) * o_data.cstep * ifm_c_align32 * maxk);

//     for (int k = 0; k < maxk; k++) {
//         for (int ic = 0; ic < inst.ifm_c; ic++) {
//             wgt_o_ptr = wgt_buffer.channel(ifm_c_align32 * k + ic);
...
//     for (int i = 0; i < ncnn::alignSize(inst.ifm_c, 32) * maxk / 32; i++) {
//         for (int k = 0; k < 32; k++) {
//             char *wgt_i_ptr = wgt_buffer.channel(i * 32 + k);
//             char *wgt_o_ptr = o_data.channel(i * 32 + 31 - k);
//             memcpy(wgt_o_ptr, wgt_i_ptr, sizeof(char) * ncnn::alignSize(inst.ofm_c, 32));
//         }
//     }
...
static void wgt_reorder(ncnn::Mat &i_data, ncnn::Mat &o_data, int ifm_c, int ofm_c, int kernel, int flip_en = 0, int op = 0) {
#ifdef MAT_LOG
    // log_origin_wgt(i_data, inst);
#endif
    ...
    if (op == 0) {
        ...
        // printf("[ log ]: idata shape(w,h,c,d) -> (%d,%d,%d,%d)\n",i_data.w,i_data.h,i_data.c,i_data.d);
        // printf("[ log ]: o_data shape(w,h,c,d) -> (%d,%d,%d,%d)\n",o_data.w,o_data.h,o_data.c,o_data.d);
        // printf("%d\n",oc_step);
        // printf("%d\n",o_data.cstep);
...
static void wgt_gen(ncnn::Mat &wgt_buffer_res, ncnn::Mat &wgt_buffer, int ifm_c, int ofm_c, int kernel, int op=0, int oc_segmentation=0) {
    int oc = ofm_c;
