...
static int detect_resnet50(const cv::Mat& bgr, std::vector<float>& cls_scores)
{
    ...
    if (resnet50.load_param_mem(resnet50.load_file2mem("models/resnet50/resnet50_int8.param")))
        exit(-1);
    if (resnet50.load_model_mem(resnet50.load_file2mem("models/resnet50/resnet50_int8.bin")))
        exit(-1);
...
int resnet50_inference() {
    ...
    // 加载 ImageNet 标签
    const char* labelPath = "models/resnet50/catagories.txt";  // 替换为实际路径
