...
static int detect_resnet18(const cv::Mat& bgr, std::vector<float>& cls_scores)
{
    ...
    if (resnet18.load_param_mem(resnet18.load_file2mem("models/resnet18/resnet18_int8.param")))
        exit(-1);
    if (resnet18.load_model_mem(resnet18.load_file2mem("models/resnet18/resnet18_int8.bin")))
        exit(-1);

    // if (resnet18.load_param_mem(resnet18.load_file2mem("models/resnet18/resnet18_opt.param")))
    //     exit(-1);
    // if (resnet18.load_model_mem(resnet18.load_file2mem("models/resnet18/resnet18_opt.bin")))
    //     exit(-1);
...
int resnet18_inference() {
    ...
    // 加载 ImageNet 标签
    const char* labelPath = "models/resnet18/catagories.txt";  // 替换为实际路径
