...
static int detect_rfcn(const cv::Mat& bgr, std::vector<Object>& objects)
{
    ...
    rfcn.opt.use_int8_inference = true;
    ...
    // if (rfcn.load_param_mem(rfcn.load_file2mem("models/rfcn/rfcn_int8.param")))
    //     exit(-1);
    // if (rfcn.load_model_mem(rfcn.load_file2mem("models/rfcn/rfcn_int8.bin")))
    //     exit(-1);

    if (rfcn.load_param_mem(rfcn.load_file2mem("models/rfcn/rfcn_opt.param")))
        exit(-1);
    if (rfcn.load_model_mem(rfcn.load_file2mem("models/rfcn/rfcn_opt.bin")))
        exit(-1);
