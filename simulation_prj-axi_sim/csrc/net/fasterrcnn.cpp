...
static int detect_fasterrcnn(const cv::Mat& bgr, std::vector<Object>& objects)
{
    ...
    // if (fasterrcnn.load_param_mem(fasterrcnn.load_file2mem("models/fasterrcnn/fasterrcnn_int8.param")))
    //     exit(-1);
    // if (fasterrcnn.load_model_mem(fasterrcnn.load_file2mem("models/fasterrcnn/fasterrcnn_int8.bin")))
    //     exit(-1);

    if (fasterrcnn.load_param_mem(fasterrcnn.load_file2mem("models/fasterrcnn/fasterrcnn_opt.param")))
        exit(-1);
    if (fasterrcnn.load_model_mem(fasterrcnn.load_file2mem("models/fasterrcnn/fasterrcnn_opt.bin")))
        exit(-1);
    // hyper parameters taken from
    // py-faster-rcnn/lib/fast_rcnn/config.py
    // py-faster-rcnn/lib/fast_rcnn/test.py
    const int target_size = 224; // __C.TEST.SCALES
