...
static int detect_yolact(const cv::Mat& bgr, std::vector<Object>& objects)
{
    ...
    yolact.opt.use_int8_inference = true;

    if (yolact.load_param_mem(yolact.load_file2mem("models/yolact/yolact_int8.param")))
        exit(-1);
    if (yolact.load_model_mem(yolact.load_file2mem("models/yolact/yolact_int8.bin")))
        exit(-1);

    // if (yolact.load_param_mem(yolact.load_file2mem("models/yolact/yolact_opt.param")))
    //     exit(-1);
    // if (yolact.load_model_mem(yolact.load_file2mem("models/yolact/yolact_opt.bin")))
    //     exit(-1);
