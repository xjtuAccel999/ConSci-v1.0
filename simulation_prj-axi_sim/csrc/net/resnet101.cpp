...
static int detect_resnet101(const cv::Mat& bgr, std::vector<float>& cls_scores)
{
    ...
    if (resnet101.load_param_mem(resnet101.load_file2mem("models/resnet101/resnet101_int8.param")))
        exit(-1);
    if (resnet101.load_model_mem(resnet101.load_file2mem("models/resnet101/resnet101_int8.bin")))
        exit(-1);
...
int resnet101_inference() {
    ...
    // 加载 ImageNet 标签
    const char* labelPath = "models/resnet101/catagories.txt";  // 替换为实际路径
