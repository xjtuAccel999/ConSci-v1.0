...
static int detect_scrfd(const cv::Mat& bgr, std::vector<FaceObject>& faceobjects)
{
    ...
    scrfd.opt.use_int8_inference = true;

    // model is converted from
    // https://github.com/deepinsight/insightface/tree/master/detection/scrfd
    // the ncnn model https://github.com/nihui/ncnn-assets/tree/master/models
    if (scrfd.load_param_mem(scrfd.load_file2mem("models/scrfd/scrfd_opt.param")))
        exit(-1);
    if (scrfd.load_model_mem(scrfd.load_file2mem("models/scrfd/scrfd_opt.bin")))
        exit(-1);

    // if (scrfd.load_param_mem(scrfd.load_file2mem("models/scrfd/scrfd_int8.param")))
    //     exit(-1);
    // if (scrfd.load_model_mem(scrfd.load_file2mem("models/scrfd/scrfd_int8.bin")))
    //     exit(-1);
