...
void forward_ncnn_hardswish(ncnn::Mat &in, ncnn::Mat &out,ncnn::ParamDict &pd) {
    ...
    int ret = test_layer<ncnn::HardSwish>("HardSwish", pd, weights, in, out);
    if (ret != 0) {
...
void forward_sim_hardswish(ncnn::Mat &in, ncnn::Mat &out,ncnn::ParamDict &pd) {
    float alpha = pd.get(0, 0.2f);
    float beta = pd.get(1, 0.5f);
    ...
    for (int q = 0; q < in.c; q++)
    {
        ...
        for (int i = 0; i < in.w*in.h; i++)
        {
            if (i_ptr[i] < lower)
                ...
                o_ptr[i] = i_ptr[i] * (i_ptr[i] * alpha + beta);
        }
    }
}
...
void test_layer_hardswish() {
    ...
    forward_sim_hardswish(data_in, c_sim, pd);
    forward_ncnn_hardswish(data_in, c_ncnn, pd);
    // printf("data_in\n");
    ...
    if (CompareMat(c_ncnn, c_sim, 0.001) == 0)
        printf("\033[;32m[ log ]: TEST HARDSWISH BETWEEN NCNN AND SIMULATION PASS\n\033[0m");
    else
        printf("\033[;31m[ log ]: TEST HARDSWISH BETWEEN NCNN AND SIMULATION FAILED\n\033[0m");
}
